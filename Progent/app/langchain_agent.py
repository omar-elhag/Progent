...
class ExecuteSpatialFunctionTool(BaseTool):
    ...
    def _run(self, tool_input_str: str) -> Dict[str, Any]:
        ...
        try:
            # Enhanced parsing to handle extra conversational text
...
def _get_declarations_stateless(function_ids_str: str) -> str:
    ...
    Enhanced to handle tendency to add extra conversational text.
    """
    try:
        ...
        # Parse the input - handle multiple formats and extra text
        function_ids = []

        if isinstance(function_ids_str, str):
            # Clean the input by removing extra conversational text that might be added
            import re

            # Look for patterns like [10], [1,2,3], 10, "10", etc.
            # Remove common additions like "Please let me know what you'd like to do next!"
...
class LangChainAgent:
    ...
    def set_model(self, model_key: str):
        ...
        # (or google_api_key param) which is not required for other models.
        if model_key.startswith("GEMINI"):
            ...
            # No adapter available for other models; do not instantiate LangChain LLM
            self.llm = None
...
Layers ({layer_count} total): {', '.join(layer_names) if layer_names else 'None'}"""
    ...
    def _create_prompt_template(self) -> PromptTemplate:
        """Creates the optimized prompt template for the agent with specific formatting rules."""
...
- "TOOLS" if the user wants GIS analysis, data manipulation, spatial operations, dashboard operations, or asks about layers/maps
